...
int main() {
  ...
  for (const auto& device : devices) {
    ...
    // Inner scope to deallocate memory before destroying the stream
    {
      ...
      SiStripMappingDevice conditions_d(100, queue);    // (the namespace specification is to avoid confusion with the non-alpaka sistrip namespace)
      testMappingSoA::runKernels(conditions_d.view(), queue);
      
      // Instantate tracks on host. This is where the data will be
      // copied to from device.
      SiStripMappingHost conditions_h(conditions_d.view().metadata().size(), queue);    // (the namespace specification is to avoid confusion with the non-alpaka sistrip namespace)
