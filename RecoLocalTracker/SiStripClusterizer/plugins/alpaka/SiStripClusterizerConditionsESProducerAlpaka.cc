...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  class SiStripClusterizerConditionsESProducerAlpaka : public ESProducer {  
    ...
    SiStripClusterizerConditionsESProducerAlpaka(edm::ParameterSet const& iConfig) : ESProducer(iConfig) {
      invthick_.reserve(sistrip::NUMBER_OF_FEDS * sistrip::FEDCH_PER_FED);
      detID_.reserve(sistrip::NUMBER_OF_FEDS * sistrip::FEDCH_PER_FED);
      iPair_.reserve(sistrip::NUMBER_OF_FEDS * sistrip::FEDCH_PER_FED);
      noise_.reserve(sistrip::NUMBER_OF_FEDS * sistrip::FEDCH_PER_FED * sistrip::STRIPS_PER_FEDCH);
      gain_.reserve(sistrip::NUMBER_OF_FEDS * sistrip::APVS_PER_FEDCH * sistrip::FEDCH_PER_FED);

      auto cc = setWhatProduced(this);      
      gainsToken_ = cc.consumesFrom<SiStripGain, SiStripGainRcd>();
      noisesToken_ = cc.consumesFrom<SiStripNoises, SiStripNoisesRcd>();
      qualityToken_ = cc.consumesFrom<SiStripQuality, SiStripQualityRcd>(edm::ESInputTag{"", iConfig.getParameter<std::string>("QualityLabel")});          
    }
    ...
    std::optional<SiStripClusterizerConditionsHost> produce(SiStripClusterizerConditionsRcd const& iRecord) {
      ...
      LogDebug("SiStripClusterizerConditionsESProducer") << "Produced a SiStripClusterizerConditions object for " << detToFeds_.size() << " modules";
      ...
      SiStripClusterizerConditionsHost product({{DetToFedsSoA_size, Data_fedch_SoA_size, Data_strip_SoA_size, Data_apv_SoA_size}}, cms::alpakatools::host());
    ...
    void SiStripClusterizerConditionsOnHost(const SiStripQuality& quality, const SiStripGain* gains, const SiStripNoises& noises);
    ...
    inline void setInvThickness(uint16_t fed, uint8_t channel, float invthick) { invthick_[channelIndex(fed, channel)] = invthick; };
  ...
  void SiStripClusterizerConditionsESProducerAlpaka::SiStripClusterizerConditionsOnHost(const SiStripQuality& quality, const SiStripGain* gains, const SiStripNoises& noises){
    // prepare conditions
  ...
  void SiStripClusterizerConditionsESProducerAlpaka::setStrip(uint16_t fed, uint8_t channel, uint16_t strip, uint16_t noise, float gain, bool bad) {
